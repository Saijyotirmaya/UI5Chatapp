    ...
    var write_num = (function make_write_num() {
      ...
      function write_num_int(type, fmt, val) {
        ...
        switch (fmt) {
          ...
          default:
            if (fmt.slice(-3) == ".00")
              return write_num_int(type, fmt.slice(0, -3), val) + ".00";
            if (fmt.slice(-2) == ".0")
              return write_num_int(type, fmt.slice(0, -2), val) + ".0";
        }
        throw new Error("unsupported format |" + fmt + "|");
      }
    ...
    function dsv_to_sheet_str(str, opts) {
      ...
      function finish_cell() {
        ...
        if (s.charCodeAt(0) == 0x3d) {
          ...
          cell.w = s;
          cell.v = v;
        } else if (!isNaN(new Date(s).getDate())) {
          cell.t = "d";
          cell.v = parseDate(s);
        } else {
          cell.t = "s";
...
*/
  ...
  function parse_xlml_xml(d, opts) {
    ...
    if (str.substr(0, 1000).indexOf("<html") >= 0)
  ...
  function parse_xlscfb(cfb, options) {
    ...
    if (options.bookProps && !options.bookSheets) WorkbookP = {};
      ...
      /* Quattro Pro 7-8 */ else if (cfb.find("PerfectOffice_MAIN"))
        WorkbookP = WK_.to_workbook(
          cfb.find("PerfectOffice_MAIN").content,
          options
        );
      /* Quattro Pro 9 */ else if (cfb.find("NativeContent_MAIN"))
        WorkbookP = WK_.to_workbook(
    ...
    if (cell.v != null)
      switch (cell.t) {
        ...
        case "n":
          ...
          else write_biff_rec(ba, 0x0003, write_BIFF2NUMBER(R, C, v));
          return;
        case "b":
        case "e":
    ...
    var write_ws = function (ws, wb, i, opts) {
      ...
      for (; R <= range.e.r; ++R) {
        ...
        for (; C <= range.e.c; ++C) {
          ...
          o.push(
            writextag("table:table-cell", writextag("text:p", textp, {}), ct)
          );
        }
        o.push("        </table:table-row>\n");
      }
  ...
  function readSync(data, opts) {
    ...
    switch ((n = firstbyte(d, o))[0]) {
      ...
      case 0x49:
        if (n[1] == 0x44) return SYLK.to_workbook(d, o);
        break;
      case 0x54:
